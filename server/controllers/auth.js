    ...
    User.findOne({ email }).exec((err, user) => {
        ...
        var mailOptions = {
          ...
          transporter.sendMail(mailOptions, function(error, info){
            if (error) {
              res.json({
                  message: 'unable to send Email. please try after some time'
              })
            } else {
              res.json({
                  message: `Email has been sent to ${email}. Follow the inctruction to acticate your account`
              })
            }
          });
...
exports.signin = (req, res) => {
    ...
    User.findOne({email}).exec((err, user) => {
        ...
        const token = jwt.sign({_id: user._id}, process.env.JWT_SECRET, {expiresIn:'50d'});
              ...
              if(err){
                ...
                transporter.sendMail(mailOptions, function(error, info){
                    if (error) {
                      ...
                      res.json({
                          message: `Email has been sent to ${email}. Follow the inctruction to reset your password`
                      })
                    }
                  });
              }
...
exports.facebookLogin = (req, res) => {
    ...
    return(
        ...
        if(email){
            ...
            return res.status(400).json({
                message: 'Email is not associated with this facebook Account'
            })
        }
    }).catch(() => {
        return res.json({
